...
compare_disk( const void *a, const void *b)
{
...
/* Top Processes Code */
compare_top( const void *a, const void *b)
{
...
int	main(int argc, char **argv)
{
	...
	int diskreadserv_found = 0;
	int diskwriteserv_found = 0;
	int	short_style = 0;
	int	nmon9 = 0;
	int	nmon12e = 0;
	int	nmon11e = 0;
	...
	if (findfirst("DISKREADSERV,T") != -1)
		diskreadserv_found = 1;
	if (findfirst("DISKWRITESERV,T") != -1)
		diskwriteserv_found = 1;
...
#define MAXDISK 150
	...
	for (j = 0; j < disksects; j++) {
		...
		rrdcreate(a_disk[j], a_disk_size[j], string2);
		sprintf(string2, j ? "diskreadserv%d" : "diskreadserv", j);
		rrdcreate(a_disk[j], a_disk_size[j], string2);
		sprintf(string2, j ? "diskwriteserv%d" : "diskwriteserv", j);
		rrdcreate(a_disk[j], a_disk_size[j], string2);
	}
	...
	if (fc_found) {
		...
		rrdgraph(a_fc, a_fc_size, "fcwrite", "fcread", AUTO, "FC Adapter Write - FCSTAT", LINE, "Kbytes per second");
	...
	for (j = 0; j < disksects; j++) {
		...
		if (diskreadserv_found) {
			sprintf(string2, j ? "diskreadserv%d" : "diskreadserv", j);
			rrdgraph(a_disk[j], a_disk_size[j], string2, string2, AUTO, "DISK Read Service Time", LINE, "Milliseconds");
		}
		if (diskwriteserv_found) {
			sprintf(string2, j ? "diskwriteserv%d" : "diskwriteserv", j);
			rrdgraph(a_disk[j], a_disk_size[j], string2, string2, AUTO, "DISK Write Service Time", LINE, "Milliseconds");
		}
	}
